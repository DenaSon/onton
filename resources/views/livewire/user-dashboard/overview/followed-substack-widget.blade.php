<x-card separator
    ...
    <x-slot name="title">
        <div class="flex items-center gap-2">
            <x-heroicon-o-rss class="w-5 h-5 text-primary group-hover:scale-110 transition-transform duration-300"/>
            <span class="text-sm font-semibold group-hover:text-primary transition-colors duration-300">
                Followed Substack Updates
            </span>
        </div>
    </x-slot>
    ...
    <div
        ...
        @forelse($substackNewsletters as $newsletter)
            <div wire:key="substack-follow-{{$newsletter->id}}"
                ...
                <p class="flex justify-between items-center text-sm font-medium text-base-content"
                   title="{{ $newsletter->subject }}">
                    <span class="text-xs text-gray-400 order-2">{{ $newsletter->vc->name ?? '' }}</span>
                    <span class="order-1">
                        {{ \Illuminate\Support\Str::limit($newsletter->subject, 50) }}
                    </span>
                </p>

                <p class="text-xs text-gray-500 mt-1">
                    Source: Substack &bull;
        ...
        @empty
            <div class="text-sm text-gray-500 py-8 text-center">
                ...
                No Substack updates from your followed VCs yet.
            </div>
        @endforelse

    </div>

    {{--    --}}{{-- Footer link if more than 10 --}}
    {{--    @if($substackNewsletters->count() > 10)--}}
    {{--        <div class="border-t border-base-200 py-4 text-right bg-base-100">--}}
    {{--            <a href="{{ route('feed.index') }}" wire:navigate.hover--}}
